...
BEGIN
    IF EXISTS (
        ...
        WHERE n.nspname = v_schema AND ro.rolname <> v_schema
    ) THEN
...
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON ALL TABLES IN SCHEMA {schema} TO {schema};
GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA {schema} TO {schema};
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA {schema} TO {schema};

ALTER DEFAULT PRIVILEGES FOR ROLE {schema} IN SCHEMA {schema} REVOKE ALL ON TABLES FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE {schema} IN SCHEMA {schema} REVOKE ALL ON SEQUENCES FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE {schema} IN SCHEMA {schema} REVOKE ALL ON FUNCTIONS FROM PUBLIC;
