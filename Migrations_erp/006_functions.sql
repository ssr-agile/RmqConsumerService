...
CREATE FUNCTION {schema}.axi_fn_getaxobjectlist(p_userroles text) RETURNS TABLE(displaydata text, caption text, name text)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
...
CREATE FUNCTION {schema}.axi_fn_getstructlist(p_roles text, p_mode text, p_structtype text) RETURNS TABLE(displaydata text, caption text, name text)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
...
CREATE FUNCTION {schema}.f_amount_to_words(m numeric, format_type text DEFAULT 'Millions'::text, pdecimals numeric DEFAULT 2) RETURNS text
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
  ...
  n bigint := 0 ;
BEGIN
  -- Handling Million/Billion Format
 
  if m != 0 then
 
	  if m - floor(m) != 0 then
		  decimalpart := 'and '||cast( floor((m - floor(m)) * case when pdecimals = 2 then 100 else 1000 end) as varchar)||'/'||case when pdecimals = 2 then '100' else '1000' end||' Only';
	  end if;
	
	  n = floor(m);
	
	  IF lower(format_type) = 'millions' THEN
	    ...
	    SELECT
	      CASE
	        ...
	        WHEN n BETWEEN 20 AND 99 THEN (SELECT Word FROM Below100 WHERE ID = n / 10) || '-' || f_amount_to_words(n % 10)
	        WHEN n BETWEEN 100 AND 999 THEN (f_amount_to_words(n / 100)) || ' Hundred ' || f_amount_to_words(n % 100)
	        WHEN n BETWEEN 1000 AND 999999 THEN (f_amount_to_words(n / 1000)) || ' Thousand ' || f_amount_to_words(n % 1000)
	        WHEN n BETWEEN 1000000 AND 999999999 THEN (f_amount_to_words(n / 1000000)) || ' Million ' || f_amount_to_words(n % 1000000)
	        WHEN n BETWEEN 1000000000 AND 999999999999 THEN (f_amount_to_words(n / 1000000000)) || ' Billion ' || f_amount_to_words(n % 1000000000)
	        ELSE 'INVALID INPUT'
	      END || ' ' || decimalpart INTO e;
	
	  -- Handling Indian Lakh/Crore Format
	  ELSE  
	    WITH Below20(Word, Id) AS
	    (
	      VALUES
	    ...
	    SELECT
	      CASE
	        WHEN n = 0 THEN ''
	        WHEN n BETWEEN 1 AND 19 THEN (SELECT Word FROM Below20 WHERE ID = n)
	        WHEN n BETWEEN 20 AND 99 THEN (SELECT Word FROM Below100 WHERE ID = n / 10) || ' ' || f_amount_to_words(n % 10, format_type)
	        WHEN n BETWEEN 100 AND 999 THEN (f_amount_to_words(n / 100, format_type)) || ' Hundred ' || f_amount_to_words(n % 100, format_type)
	        WHEN n BETWEEN 1000 AND 99999 THEN (f_amount_to_words(n / 1000, format_type)) || ' Thousand ' || f_amount_to_words(n % 1000, format_type)
	        WHEN n BETWEEN 100000 AND 9999999 THEN (f_amount_to_words(n / 100000, format_type)) || ' Lakh ' || f_amount_to_words(n % 100000, format_type)
	        WHEN n BETWEEN 10000000 AND 999999999 THEN (f_amount_to_words(n / 10000000, format_type)) || ' Crore ' || f_amount_to_words(n % 10000000, format_type)
	        ELSE 'INVALID INPUT'
	      END || ' ' || decimalpart INTO e;
	  END IF;
  else
...
CREATE FUNCTION {schema}.fn_account_tree(pcompanyid numeric) RETURNS TABLE(sno numeric, path text, mg_accountid numeric, accountname character varying, accountcode character varying, account character varying, groupname character varying, alie character varying, subledgername text, atype character varying, primarygroup character varying, collapse text, grplevel numeric, aprefix character varying, cpath text, addable character varying, company numeric, company_code character varying, acategory character varying, ibaccount character varying, controlaccount character varying, active character varying, common_integration_code numeric, applicablebranches character varying, groupcode character varying, transid character varying)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/*
declare
--    var_r record;
    musername varchar := pusername;
    mynumber numeric := 1;
	mclientcode varchar := pclientcode;
*/	
begin		
		RETURN QUERY (
...
--            mg_account.plcategory,
            ...
            mg_account.ibaccount,
            mg_account.controlaccount,
            mg_account.active,
            mg_account.common_intergration_code,
			mg_account.applicablebranches,
			mg_account.accountcode as groupcode,
			mg_account.transid
           FROM mg_account
          WHERE mg_account.groupname IS NULL AND mg_account.company = pcompanyid
        UNION
...
--            c1.plcategory,
            ...
            c1.controlaccount,
            c1.active,
            c1.common_intergration_code,
			c1.applicablebranches,
			c2.accountcode as groupcode,
			c1.transid
           FROM mg_account c1,
            comp c2
          WHERE c1.groupname::text = c2.accountname::text AND c1.company = pcompanyid
        )
		SELECT
		row_number() OVER (ORDER BY a.path)::numeric AS sno,
	    a.path,
	    a.mg_accountid,
	    a.accountname,
	    a.accountcode,
	        CASE
--	            WHEN a.grplevel > 0::numeric THEN (repeat('.'::text, ( (a.grplevel - 1) * 5::numeric - 1::numeric)::integer)
				WHEN a.grplevel > 0::numeric THEN (repeat('.'::text, ( (a.grplevel - 1) * 5::numeric)::integer)
	            || a.accountname::text)::character varying
	            ELSE a.accountname
	        END AS account,
	    a.groupname,
	    a.alie,
--	    a.plcategory,
        ...
        a.controlaccount,
        a.active,
        a.common_intergration_code,
		a.applicablebranches,
		a.groupcode,
		a.transid
	   FROM comp a
	  ORDER BY a.alie, a.path, a.depth	  
	 ) ;	
end;
$$;

CREATE FUNCTION {schema}.fn_accounts_balances(pcompanyid numeric, pbranchid numeric, pperiod text, pparentid numeric, pfromdate date, ptodate date, psummaryflag character varying DEFAULT 'F'::character varying, pcumulative character varying DEFAULT 'F'::character varying) RETURNS TABLE(ordno numeric, parentid numeric, period_text text, mperiod_startdate date, mperiod_enddate date, mperiod_subtext character varying, mg_accountid numeric, balance numeric)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/*
...
begin			
	RETURN QUERY
			(	
			select ROW_NUMBER () OVER (partition by x.parentid, x.mg_accountid ORDER BY x.parentid, x.mg_accountid, x.period_startdate)::numeric as ordno,
			x.parentid::numeric, x.period_text::text, x.period_startdate::date, x.period_enddate::date,
			x.period_subtext::character varying, x.mg_accountid::numeric, x.balance::numeric
			from (
			select m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			m.childid as mg_accountid,
			sum(coalesce(n.base_debit,0)-coalesce(n.base_credit,0)) as balance
			from (
				select a.*, b.*
				from
				(select a.parantid as parentid, a.childid
				from vw_accounttreelink a
				where  a.parantid = pparentid
				union
				select pparentid as parentid, 0 as childid from dual
				) a,		
				(select distinct v.period_startdate, v.period_enddate,
				v.period_text as period_text,
				v.period_subtext,
				v.period_rangefrom, v.period_rangeto	
				from vw_companycalendarrows v where ( v.period_range = pperiod or pperiod = 'DEFAULT')
				and v.period_rangefrom >= cast(to_char( cast( pfromdate as date),'YYYYMM') as numeric)
				and v.period_rangeto <= cast(to_char( cast( ptodate as date),'YYYYMM') as numeric)
				and v.company = pcompanyid
				and pperiod != 'DEFAULT'
				union
				select cast( pfromdate as date) period_startdate, cast( ptodate as date) period_enddate,
				pperiod as period_text,
				to_char( cast( pfromdate as date),'dd/mm/yy')||'-'||to_char( cast( ptodate as date),'dd/mm/yy')  as period_subtext,
				cast(to_char( cast( pfromdate as date),'YYYYMM') as numeric) period_rangefrom,
				cast(to_char( cast( ptodate as date),'YYYYMM') as numeric) period_rangeto	
				where pperiod = 'DEFAULT'
				) b		
			) m left join mv_accountsdetails n on ( m.childid = n.mg_accountid and n.voucher_date between m.period_startdate and m.period_enddate )
			group by m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			m.childid
			) x
			where psummaryflag = 'F' and pcumulative = 'F'			
			union all			
			select 60 as ordno, m.parentid, 'Totals'::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			case when psummaryflag = 'F' then m.childid	else 0 end as mg_accountid,
			sum(coalesce(n.base_debit,0)-coalesce(n.base_credit,0)) as balance
			from (
				select a.*, b.*
				from
			...
			where pcumulative = 'F'
			group by m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			case when psummaryflag = 'F' then m.childid	else 0 end			
			union all 						
			select ROW_NUMBER () OVER (partition by x.parentid ORDER BY x.parentid, x.period_startdate)::numeric as ordno,
			x.parentid::numeric, x.period_text::character varying, x.period_startdate::date, x.period_enddate::date, x.period_subtext::character varying, x.mg_accountid::numeric,
			x.balance::numeric
			from (
			select m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			0 as mg_accountid,
			...
			where psummaryflag = 'T' and pcumulative = 'F'			
-- CUMULATIVE			
			union all
			select ROW_NUMBER () OVER (partition by x.parentid, x.mg_accountid ORDER BY x.parentid, x.mg_accountid, x.period_startdate)::numeric as ordno,
			x.parentid::numeric, x.period_text::text, x.period_startdate::date, x.period_enddate::date, x.period_subtext::character varying, x.mg_accountid::numeric, x.balance::numeric
			from (
			select m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			m.childid as mg_accountid,
			sum(coalesce(n.base_debit,0)-coalesce(n.base_credit,0)) as balance
			...
			) m left join mv_accountsdetails n on ( m.childid = n.mg_accountid and n.voucher_date <= m.period_enddate )
			group by m.parentid, m.period_text::character varying, m.period_startdate::date, m.period_enddate::date, m.period_subtext::character varying,
			m.childid
			) x
			where psummaryflag = 'F' and pcumulative = 'T'			
			union all						
			select ROW_NUMBER () OVER (partition by x.parentid ORDER BY x.parentid, x.period_startdate)::numeric as ordno,
			x.parentid::numeric, x.period_text::character varying, x.period_startdate::date, x.period_enddate::date, x.period_subtext::character varying, x.mg_accountid::numeric,
			x.balance::numeric
			...
			where psummaryflag = 'T' and pcumulative = 'T'									
			order by parentid, mg_accountid, period_startdate			
		);			
end;
$$;

CREATE FUNCTION {schema}.fn_accounts_branchlist(pcompanyid numeric, pbranchid numeric, pusername character, pdocdate date) RETURNS TABLE(mg_accountid numeric, accountname character varying, accountcode character varying, acategory character varying, acurrency numeric, controlaccount character varying, alie character varying, cc1app character varying, cc2app character varying, cc3app character varying, cc4app character varying, exrate numeric, company numeric, accountcode_alias character varying, group_accountcodes character varying, currency character varying)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/*
...
begin		
	RETURN QUERY (
		...
		a0.mg_accountid,
		a0.accountname,
		a0.accountcode_alias,
		a0.acatcode as acategory,
		a0.acurrency,
		a0.controlaccount,
		a0.alie,
		a0.cc1app,
		...
		a0.cc3app,
		a0.cc4app,
		get_exrate( cast( pcompanyid as numeric) ,
		a0.acurrency,
		cast('T' as varchar), cast( pdocdate as date) ) exrate,
		a0.company,
		a0.accountcode_alias,
		a0.group_accountcodes,
		a1.currency
	from
		mg_account a0, currency a1
	where a0.applicablebranches = 'ALL'
		and a0.company = pcompanyid
		and a0.atype = 'Account'
		and a0.Active = 'T'
		and a0.cancel = 'F'
		and a0.app_desc = 1
		and a0.acurrency = a1.currencyid
		and exists (
		select
			'x'
		from vw_userbranchlocations x
		where
		    x.username = pusername
		and x.companyid = pcompanyid
		and x.branchid  = pbranchid)
	order by
	a0.AccountName
	);	
end;
...
CREATE FUNCTION {schema}.fn_accounts_justbalances(pcompanyid numeric, pbranchid numeric DEFAULT 1, pcommonintegratiocode numeric DEFAULT 0, pfromdate date DEFAULT CURRENT_DATE, ptodate date DEFAULT CURRENT_DATE, pcumulative text DEFAULT 'F'::text) RETURNS numeric
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
declare
    maccountbalances numeric := 0;	
begin			 
	select sum(b.base_debit-b.base_credit) into  maccountbalances
	from vw_accounttreelink a,  mv_accountsdetails b   
	where a.parantid in 				
	(select a.accountname
	from vw_parantlink  a
	where  ( a.common_intergration_code =  pcommonintegratiocode or a.common_intergration_code1 =  pcommonintegratiocode  
	or a.common_intergration_code2 =  pcommonintegratiocode)
	and a.company = pcompanyid
	)  
	and a.parantid = b.mg_accountid
	and b.company = cast( pcompanyid as numeric)
	and ( b.branchid = cast( pbranchid as numeric) or ( cast( pbranchid as numeric) = 0 or cast( pbranchid as numeric) = 1)  )
	and
	(
	( b.voucher_date between cast( pfromdate as date) and  cast( ptodate as date) and pcumulative = 'F') or
    ( b.voucher_date <= cast( ptodate as date) and pcumulative = 'T')
    )	;
    return maccountbalances; 
end;
$$;
...
CREATE FUNCTION {schema}.fn_axi_getstructs_obj(pcmd character varying, pusername character varying, puserrole character varying, ptransid character varying, pselectedfield character varying, pdimension character varying, ppermission character varying, pkeyfield character varying, pprimarytable character varying, pglobalvars character varying) RETURNS TABLE(displaydata text, id text, caption text, isfield text)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $_$
declare 
...
CREATE FUNCTION {schema}.fn_axi_getstructures_meta(pusername character varying, puserrole character varying, presponsiblity character varying, pmode character varying, pstype character varying) RETURNS TABLE(displaydata character varying, caption character varying, name character varying, stype character varying, dimension character varying, permission character varying, createallowed character varying, viewallowed character varying, keyfield character varying, primarytable character varying)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
...
CREATE FUNCTION {schema}.fn_bom_treev2(pcompanyid numeric, pitemid numeric, pversion character varying DEFAULT '001'::character varying, pdqty numeric DEFAULT 1, psalesorder character varying DEFAULT 'NA'::character varying, ptype character varying DEFAULT 'f'::character varying) RETURNS TABLE(sno numeric, path text, bomheaderid numeric, itemdesc character varying, itemcode character varying, legacyitemcode character varying, bomname character varying, uom character varying, itemid numeric, active character varying, cpath text, grplevel numeric, isbom character varying, bomdetailsid numeric, bomid character varying, aqty numeric, qty numeric, demandqty numeric, parent_item text, salesorder character varying)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
    IF lower(ptype) = 'f' THEN
        IF NOT EXISTS (
            SELECT 1 FROM bomheader a
            WHERE a.active = 'T' AND a.company = pcompanyid
              AND a.salesorder_number = psalesorder AND a.bitemname = pitemid
        ) THEN
            psalesorder := 'NA';
            pversion := '001';
        ELSE
            SELECT right(max(a.bomid), 3) INTO pversion
            FROM bomheader a
            WHERE a.active = 'T' AND a.company = pcompanyid
              AND a.salesorder_number = psalesorder AND a.bitemname = pitemid;
        END IF;
    ...
    RETURN QUERY (
        WITH RECURSIVE boms AS (
            ...
            SELECT
                ...
                b.bomid,
                b.bqty AS aqty,
                CAST(b.bqty AS numeric(15,3)) AS qty,         -- ✅ cast added
                CAST(pdqty AS numeric(15,3)) AS demandqty,    -- ✅ cast added
                (i.itemid || '-' || b.bomheaderid) || ''::text AS path,
                i.itemdesc || ''::text AS cpath,
                0 AS grplevel,
                i.isbom,
                0::numeric AS bomdetailsid
            FROM bomheader b
            JOIN item i ON b.bitemname = i.itemid
            WHERE i.company = pcompanyid
              AND b.salesorder_number = psalesorder
              AND i.itemid = pitemid
              AND (b.bomid LIKE '%' || pversion OR pversion = (CASE WHEN b.bomheaderid IS NULL THEN '001' END))
              AND i.cancel = 'F'
              AND i.app_desc = 1
              AND i.active = 'T'
              AND i.isbom = 'T'
            ...
            SELECT
                COALESCE(
                    (SELECT bh.bomheaderid FROM bomheader bh
                     WHERE bh.bitemname = i.itemid AND bh.company = pcompanyid
                       AND bh.salesorder_number = s.salesorder_number LIMIT 1),
                    b.bomheaderid
                ) AS bomheaderid,
                i.itemdesc,
                b.salesorder_number,
                ...
                i.itemcode,
                s.depth + 1 AS depth,
                CASE WHEN s.depth = 0 THEN b.bomid ELSE s.bomid END AS bomid,
                d.qty AS aqty,
                CAST(s.qty * d.qty AS numeric(15,3)) AS qty,
                CAST(s.qty * pdqty * d.qty AS numeric(15,3)) AS demandqty,  -- ✅ cast added
                (((s.path || ','::text) || i.itemid::text) || '-'::text) || d.bomdetailsid AS path,
                (s.cpath || '~'::text) || i.itemdesc::text AS cpath,
                s.grplevel + 1 AS grplevel,
                i.isbom,
                d.bomdetailsid
            FROM boms s
            JOIN bomheader b ON s.itemid = b.bitemname
            JOIN bomdetails d ON b.bomheaderid = d.bomheaderid
            JOIN item i ON i.itemid = d.itemname
            WHERE b.company = pcompanyid
              AND b.salesorder_number = (
                  CASE WHEN EXISTS (
                      SELECT 1 FROM bomheader a
                      WHERE a.active = 'T' AND a.company = pcompanyid
                        AND a.salesorder_number = psalesorder AND a.bitemname = s.itemid
                  ) THEN psalesorder ELSE 'NA' END
              )
              AND i.cancel = 'F'
              AND i.app_desc = 1
              AND i.active = 'T'
              AND s.path NOT LIKE '%' || i.itemcode || '-' || d.bomdetailsid || '%'
        )
        SELECT DISTINCT ON (a.path)
            ...
            a.itemcode,
            a.legacyitemcode,
            CASE WHEN a.grplevel > 0 THEN
                (repeat('.'::text, ((a.grplevel - 1) * 5)::integer) || a.itemdesc::text)::character varying
            ELSE a.itemdesc END AS bomname,
            a.uom,
            a.itemid,
            a.active,
            a.cpath,
            ...
            a.qty,
            a.demandqty,
            CASE WHEN a.grplevel = 0 THEN 'None'
                ELSE (string_to_array(a.cpath, '~'))[array_length(string_to_array(a.cpath, '~'), 1) - 1]
            END AS parent_item,
            a.salesorder_number
        FROM boms a
        ORDER BY a.path, a.depth, a.itemdesc
    );
END;
$$;
...
CREATE FUNCTION {schema}.fn_findheaderid(ptransid character varying, pgridtableid numeric DEFAULT 1, pdc numeric DEFAULT 2) RETURNS numeric
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
declare
    rheaderid numeric default 0;
	rheadertable text;
	sqlstring text;
begin
	
	select tablename||'id' into rheadertable from axpdc a where a.tstruct = ptransid and a.dname = 'dc1';
	
	select 'select '||rheadertable||' from '||a.tablename||' where '||a.tablename||'id='||pgridtableid into sqlstring
	from axpdc a where a.tstruct = ptransid and a.dname = cast( 'dc'||cast( pdc as text) as text);
   	if sqlstring is not null then
		execute sqlstring into rheaderid;
	end if;
    return coalesce(rheaderid,0);
end; $$;

CREATE FUNCTION {schema}.fn_fulfill_order(plocation character varying, pcompany numeric, orderno character varying) RETURNS character varying
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
    j RECORD;
   	...
   	available varchar(2);
BEGIN
	drop table  if exists temp_item_list;
    create temp table temp_item_list as
				(SELECT b1.itemname , b1.itemdesc as itemid ,  a1.inward - a1.outward-COALESCE(a1.reserved_qty  , 0) AS stock_qty,b1.zishtaitemcode
          		FROM batchmaster a1 join item b1 ON a1.itemname  = b1.itemid  join location l on a1.location = l.locationid
            	where a1.cancel ='F' and a1.app_desc =1 and (a1.inward - a1.outward -a1.reserved_qty ) > 0 and a1.company  = pcompany
            	and lower(l.locationname) = lower(cast(  plocation as varchar))
            	order by b1.itemdesc);            
    FOR j IN (select a.so_portalhdrid ,a.docdate, a.docid,a.order_number , i.itemdesc as itemname, b.rate, b.order_qty ,a.sales_person,
    			coalesce(i.itemdesc,b.itemname) as itemdesc,b.zishtaitemcode, coalesce(i.itemcode  ,'CI') as itemcode,b.so_portaldtlid,a.finance_status,
    			i.itemid as itemnumber,br.branchid branch from so_portalhdr a join so_portaldtl b on a.so_portalhdrid= b.so_portalhdrid
    				left outer join item i on b.zishtaitemcode = i.zishtaitemcode  
					left join branch br on a.branch =br.branchid
  					where  a.cancel ='F' and a.app_desc =1 and a.order_number = orderno  
              		ORDER BY a.order_number, a.docdate )
	loop
			select k.stock_qty into stock from temp_item_list k where k.zishtaitemcode =  j.zishtaitemcode;
				if j.order_qty <= stock then
					available :='Y';
				else
					return 'N';
				end if;				
...
CREATE FUNCTION {schema}.fn_getledgerdata(precordid numeric, paccountname numeric, pdrcr text, pdetailed text DEFAULT 'T'::text, pcostcentre text DEFAULT 'F'::text, pvdim1 text DEFAULT NULL::text, pvdim2 text DEFAULT NULL::text, pvdim3 text DEFAULT NULL::text, pvdim4 text DEFAULT NULL::text) RETURNS text
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
...
begin
/* CONCAT(
'<span style="font-weight:bold;">', 'Scheme/Fund Code ', '</span>', b.scheme_code||'-'||b.fund_code, '<br/>',
'<span style="font-weight:bold;">', 'UCC Code ', '</span>', b.ucc_code, '<br/>',
'<span style="font-weight:bold;">', 'Client Id ', '</span>', b.account_client_id, '<br/>',
*/ 		
	select string_agg( a.details, chr(10) )  into l_result
		from (
		select rpad(m.accountname,40,' ')||' '||case when
		b.bdbamount = 0 then 'Cr '||trim(to_char(b.bcramount,'999,999,999.99'))::text
		else
		'Dr '||trim(to_char(b.bdbamount,'999,999,999.99'))::text end
		||' '||b.narration
		as details
		from accountsdtl b, accountshdr a, mg_account m
		where a.accountshdrid = b.accountshdrid and
		b.accountname = m.mg_accountid and
		a.accountshdrid = precordid and
		b.accountname != paccountname	
	) a;    
    return l_result;
end;
$$;
...
select sqltext,case when smartlistcnd like '%Dynamic select columns%' then 'T' else 'T' end dynamiccolumns,
case when smartlistcnd like '%Filter%' then 'T' else 'T' end filters,
'T' pagination,
case when smartlistcnd like '%Sorting%' then 'T' else 'T' end sorting,
case when smartlistcnd like '%Grouping%' then 'T' else 'T' end aggregations,
applydimensions
into v_adssql,v_addcolselection,v_addfilter,v_addpagination,v_addsoring,v_grouping,v_dimensions 
from axdirectsql 
...
CREATE FUNCTION {schema}.fn_refresh_mv_accountmonthlysummary() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
    REFRESH MATERIALIZED VIEW MV_ACCOUNTMONTHLYSUMMARY;
    RETURN NEW;
END;
$$;
...
CREATE FUNCTION {schema}.fn_smartview_metadata(p_svname character varying) RETURNS TABLE(sv_name character varying, sv_caption character varying, sv_sourcecnd integer, sqlname character varying, api_config text, memdb_key character varying, allowedit character varying, editmode character varying, allownew character varying, newforms character varying, newforms_transid character varying, bulksave character varying, dataupload character varying, validatedata character varying, sv_keycol character varying, savemode character varying, fldname character varying, fldcaption character varying, fdatatype character varying, normalized character varying, sourcetable character varying, sourcefld character varying, hyp_structtype character varying, hyp_transid character varying, tbl_hyperlink text, hyp_inline character varying, dynamiccolumns character varying, filters character varying, pagination character varying, sorting character varying, col_hide character varying, col_filter character varying, keyfield character varying, displaydata character varying, name character varying, caption character varying)
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
begin
...
CREATE FUNCTION {schema}.func_accvouchercreationupdate() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/* ALTERED ON 04 MAY 2025 BY VAIDHEESH*/ 
/* REFER TO OLD VERSION func_stockallocanew() 
declare 				
   mflag numeric:= 0;
   mbranchid numeric:= 0;
   mitemdesc text;
   mbatchlastnumber numeric:=0;
   mitemcode varchar(20);
   mitemcodebatch varchar(20):='';
   minvmethod varchar(1) := '1'; 
   batch_record RECORD;
*/
declare 
	maccountshdrid numeric :=  0;
	mstockaccount numeric :=0;
	mstockincdecaccount numeric :=0;
	mnewbalance numeric :=0;
	mcurrencyid numeric :=0;
	mdebit numeric := 0;
	mcredit numeric := 0;
	mstockposting varchar(1) :='T'; 
--	mcounter varchar(2) := '';
begin		 

--	select coalesce(stockposting,'F') into mstockposting from company a where a.companyid = new.mcompany;
	
	if mstockposting = 'T' then
	
		SELECT mg_accountid, acurrency  into mstockaccount, mcurrencyid FROM mg_account a WHERE a.company = new.company and a.common_intergration_code =
								 1381440000000 and a.atype = 'Account';
	
		SELECT mg_accountid into mstockincdecaccount FROM mg_account a WHERE a.company = new.company and a.common_intergration_code = 1474550000000 and a.atype = 'Account';
	 	
		if TG_OP = 'INSERT' then
	
/*				select accountshdrid into maccountshdrid from accountshdr a where 
					a.company = new.company and
					a.branch = new.branch and  
					a.subtypename = 1913110000000 and
					a.docdate = new.docdate;
*/
				REFRESH MATERIALIZED VIEW mv_accountsdetails;					
				REFRESH MATERIALIZED VIEW mv_accountmonthlysummary ;

				select  
				case when sum(a.openingbalance-a.closingvalue) <0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end,  
				case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end 
				into mdebit, mcredit
				from (
				select ma.accountname,sum(ma.base_debit-ma.base_credit) as openingbalance, 0 as closingvalue
				from mv_accountsdetails ma  
				where ma.acatcode = 's' and ma.branchid = new.branchid
				and ma.voucher_date <= new.docdate
				group by ma.accountname, ma.branchname
				union
				select i.stockac, 0 as openingbalance, sum( a.calculated_stockvalue  ) as closingvalue 
				from stockvalue a, item i 
				where a.cancel = 'F' 
				and a.itemname = i.itemid 
				and a.branch = new.branch
				and a.docdate <= new.docdate
				group by i.stockac) a 
				having (sum(a.closingvalue) != 0 or sum(a.openingbalance) != 0);

	
				if ( coalesce(mdebit,0) != 0 or coalesce(mcredit,0) != 0)  then 

					select nextval( 'inbound_inboundid_seq' ) into maccountshdrid;

					insert into accountsdtl (accountsdtlid, accountshdrid, accountsdtlrow, accountname, acategory, acurrency, aexrate,  
											  subledgername,
											dbamount, cramount, bdbamount, bcramount, ndbamount, ncramount)			
					(select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 1 as accountsdtlrow, 
					a.mg_accountid as accountname, 's' as acategory, (select acurrency from mg_account mg where mg.mg_accountid = a.mg_accountid) as acurrency, 1 as aexrate,  1 as subledgername,
--					sum(a.openingbalance) as openingbalance, sum(a.closingvalue) as closingvalue, 
					case when sum(a.openingbalance-a.closingvalue) <0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as dbamount, 
					case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as cramount,
					case when sum(a.openingbalance-a.closingvalue) <0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as bdbamount, 
					case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as bcramount,
					case when sum(a.openingbalance-a.closingvalue) <0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as ndbamount, 
					case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as ncramount
					from (
						select ma.mg_accountid,sum(ma.base_debit-ma.base_credit) as openingbalance, 0 as closingvalue
						from mv_accountsdetails ma  
						where ma.acatcode = 's' and ma.branchid = new.branchid and ma.voucher_date <= new.docdate
						group by ma.mg_accountid
						union all
						select m.mg_accountid, 0 as openingbalance, sum( a.calculated_stockvalue  ) as closingvalue 
						from stockvalue a, item i, mg_account m
						where a.cancel = 'F' 
						and a.itemname = i.itemid 
						and i.stockac = m.accountname
						and a.branch = new.branchid
						and m.company = new.company					
						and a.docdate <= new.docdate
						group by m.mg_accountid) a
					group by a.mg_accountid 
					having (sum(a.closingvalue) != 0 or sum(a.openingbalance) != 0)
					);						
					insert into accountsdtl (accountsdtlid, accountshdrid, accountsdtlrow, accountname, acategory, acurrency, aexrate,  
											  subledgername,
											dbamount, cramount, bdbamount, bcramount, ndbamount, ncramount)			
					(select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 1 as accountsdtlrow, 
					mstockincdecaccount as accountname, 's' as acategory, (select acurrency from mg_account mg where mg.mg_accountid = a.mg_accountid) as acurrency, 1 as aexrate,  1 as subledgername,
--					sum(a.openingbalance) as openingbalance, sum(a.closingvalue) as closingvalue, 
					case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as dbamount, 
					case when sum(a.openingbalance-a.closingvalue) <0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as cramount,
					case when sum(a.openingbalance-a.closingvalue) >0 then abs(sum(a.openingbalance-a.closingvalue)) else 0 end as bdbamount, 
					...
					select acurrency into mcurrencyid from accountsdtl mg where mg.accountshdrid = maccountshdrid limit 1;
					
					insert into accountshdr 
					(accountshdrid, cancel, sourceid, mapname, username, modifiedon, createdby, createdon, wkid, 
								app_level, app_desc, app_slevel, cancelremarks, wfroles, transid, company, branch, finyr, subtypename, docdate, docid, 
								currency, exrate, refno, refdate, reversed, reverseondate, remarks, hnarration, postarap, cheque_number, cheque_date, 
								totdbamount, totcramount, supply_type, reconciled, reconciled_date)
					(SELECT maccountshdrid, new.cancel, new.stockclosing_postingid, new.transid||'10' mapname, new.username, new.modifiedon, 
								new.createdby, new.createdon, new.wkid, new.app_level, new.app_desc, new.app_slevel, 
								new.cancelremarks, new.wfroles, 'vchtr', new.company, new.branch, new.finyr finyr, new.subtypename subtypename, 
								new.docdate, new.docid as docid, mcurrencyid currency, 1 exrate, new.docid refno, null refdate, 
								null reversed, null reverseondate, null remarks, null hnarration, 0 postarap, null cheque_number, null cheque_date, 
								mdebit totdbamount, mcredit totcramount, null supply_type, 'F' reconciled, 
								null reconciled_date);	

	/*
					mnewbalance = new.totalamount;
	
					mdebit = case when mnewbalance > 0 then mnewbalance else 0 end;
					mcredit = case when mnewbalance < 0 then abs(mnewbalance) else 0 end;
	
					insert into accountsdtl (accountsdtlid, accountshdrid, accountsdtlrow, accountname, acategory, acurrency, aexrate, controlaccount, 
											  subledgername,
											dbamount, cramount, bdbamount, bcramount, ndbamount, ncramount)
					(select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 1 as accountsdtlrow, mstockaccount as accountname, 'o' as acategory, 
									mcurrencyid as acurrency, 1 as aexrate, 'F' as controlaccount, 1 as subledgername,
									mdebit as dbamount, mcredit as cramount,mdebit as bdbamount, mcredit as bcramount,mdebit as ndbamount, mcredit as ncramount
					union all
					select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 2 as accountsdtlrow, mstockincdecaccount as accountname, 'o' as acategory, 
									mcurrencyid as acurrency, 1 as aexrate, 'F' as controlaccount, 1 as subledgername,
									mcredit as dbamount, mdebit as cramount,mcredit as bdbamount, mdebit as bcramount,mcredit as ndbamount, mdebit as ncramount
					);
				else
					update accountshdr a set username = new.username, modifiedon= new.modifiedon 
					where a.accountshdrid = maccountshdrid;
	
					select dbamount - cramount + coalesce( new.totalamount,0) into mnewbalance from accountsdtl a where accountshdrid = maccountshdrid and a.accountname = mstockaccount;
	
					mdebit = case when mnewbalance > 0 then mnewbalance else 0 end;
					mcredit = case when mnewbalance < 0 then abs(mnewbalance) else 0 end;
			
					update accountsdtl a set dbamount = mdebit, cramount = mcredit, bdbamount = mdebit, bcramount = mcredit, ndbamount = mdebit, ncramount = mcredit
					where a.accountshdrid = maccountshdrid and a.accountname = mstockaccount;
	
					update accountsdtl a set dbamount = mcredit, cramount = mdebit, bdbamount = mcredit, bcramount = mdebit, ndbamount = mcredit, ncramount = mdebit
					where a.accountshdrid = maccountshdrid and a.accountname = mstockincdecaccount;											
*/
				end if;
/*				
		ELSEIF TG_OP = 'UPDATE' then		 
	
			if old.totalamount != new.totalamount and old.mdocdate = new.mdocdate then  -- ONLY CHANGE IN AMOUNT IN UPDATE
		
					update accountshdr a set username = new.username, modifiedon= new.modifiedon 
					where a.accountshdrid = maccountshdrid;
	
					select dbamount - cramount + coalesce( new.totalamount,0) - coalesce( old.totalamount,0) into mnewbalance  from accountsdtl a 
					where accountshdrid = maccountshdrid and a.accountname = mstockaccount;
			...
			elseif old.mdocdate != new.mdocdate then -- ONLY CHANGE IN TRANSACTION DATE THUS REVERTING ORIGINAL TRANSACTION AND CREATE / UPDATING NEW TRANSACTION
	
					select accountshdrid into maccountshdrid from accountshdr a where 
					a.company = new.mcompany and
					a.branch = new.mbranch and  
					a.subtypename = 1913110000000 and
					a.docdate = old.mdocdate;
	
					if coalesce(maccountshdrid,0) != 0 then 
	
						select dbamount - cramount - coalesce( old.totalamount,0) into mnewbalance from accountsdtl a 
						where accountshdrid = maccountshdrid and a.accountname = mstockaccount;
					...
					a.branch = new.mbranch and  
					a.subtypename = 1913110000000 and
					a.docdate = new.mdocdate;
	
					if coalesce(maccountshdrid,0) = 0 then 
 
						select nextval( 'inbound_inboundid_seq' ) into maccountshdrid;

--						SELECT floor(random() * (99 - 10 + 1)) + 10 into mcounter;
	
						insert into accountshdr (accountshdrid, cancel, sourceid, mapname, username, modifiedon, createdby, createdon, wkid, 
								app_level, app_desc, app_slevel, cancelremarks, wfroles, transid, company, branch, finyr, subtypename, docdate, docid, 
								currency, exrate, refno, refdate, reversed, reverseondate, remarks, hnarration, postarap, cheque_number, cheque_date, 
								totdbamount, totcramount, supply_type, reconciled, reconciled_date, checkbalance)
						(SELECT maccountshdrid, new.cancel, new.sourceid, new.transid||'10' mapname, new.username, new.modifiedon, 
								new.createdby, new.createdon, new.wkid, new.app_level, new.app_desc, new.app_slevel, 
								new.cancelremarks, new.wfroles, new.transid, new.mcompany, new.mbranch, '' finyr, 1913110000000 subtypename, 
								new.mdocdate, new.tdocid as docid, mcurrencyid currency, 1 exrate, new.mdocid refno, null refdate, 
								null reversed, null reverseondate, null remarks, null hnarration, 0 postarap, null cheque_number, null cheque_date, 
								abs(new.totalamount) totdbamount, abs(new.totalamount) totcramount, null supply_type, 'F' reconciled, 
								null reconciled_date, null checkbalance);	
						...
						insert into accountsdtl (accountsdtlid, accountshdrid, accountsdtlrow, accountname, acategory, acurrency, aexrate, controlaccount, 
											 cc1app, cc2app, cc3app, cc4app, subledgername,
											dbamount, cramount, bdbamount, bcramount, ndbamount, ncramount)
						(select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 1 as accountsdtlrow, mstockaccount as accountname, 'o' as acategory, 
									mcurrencyid as acurrency, 1 as aexrate, 'F' as controlaccount, 'F' cc1app, 'F' cc2app, 'F' cc3app, 'F' cc4app, 1 as subledgername,
									mdebit as dbamount, mcredit as cramount,mdebit as bdbamount, mcredit as bcramount,mdebit as ndbamount, mcredit as ncramount
						union all
						select nextval( 'inbound_inboundid_seq' ) as accountsdtlid, maccountshdrid, 2 as accountsdtlrow, mstockincdecaccount as accountname, 'o' as acategory, 
									mcurrencyid as acurrency, 1 as aexrate, 'F' as controlaccount, 'F' cc1app, 'F' cc2app, 'F' cc3app, 'F' cc4app, 1 as subledgername,
   ...
   IF TG_OP = 'DELETE' THEN
        RETURN OLD;  -- Use OLD for DELETE triggers
    ELSE
        RETURN NEW;
    END IF;   
...
CREATE FUNCTION {schema}.func_axglovar_function() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
    NEW.username := new.axglo_user;
	new.fromuserlogin := 'F';
    RETURN NEW;
END;
$$;

CREATE FUNCTION {schema}.func_createbatchcost() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/* ALTERED ON 21ST MARCH 2025 BY VAIDHEESH FOR DECODING BATCHES AND CREATE A INWARD HISTORY
 PRVIOUS OF THIS FUNCTION IS func_createbatchcost_v3 
declare 				
   mflag numeric:= 0;
   mbranchid numeric:= 0;
   mitemdesc text;
...
--		insert into temp (oldvalue, newvalue) values ( old.batchserialbreakup, new.batchserialbreakup);

		insert into batchandcost (batchandcostid, cancel, sourceid, mapname, username, modifiedon, createdby, createdon, wkid, app_level, app_desc, app_slevel, cancelremarks, wfroles, 
		company, itemname, branch, batch, expiry_date, batchunique, rate, qty, mg_supplierid, docid, location, transid, 
        grn_itemsrow )
		(
			select nextval( 'inbound_inboundid_seq' ), b.cancel, a.grn_itemsid sourceid, 'tsgrn'||'10' mapname, b.username, b.modifiedon, 
				b.createdby, b.createdon, b.wkid, b.app_level, b.app_desc, b.app_slevel, b.cancelremarks, b.wfroles, 
				b.company, a.itemname, b.branch,
				cast(SPLIT_PART(a.dimension_data,'|',1) as text) as batch,
				to_date(CAST(SPLIT_PART(a.dimension_data, '|', 3) AS text),'dd/mm/yyyy') as expiry_date,
				concat( cast(company as varchar), cast( itemname as varchar), cast(SPLIT_PART(a.dimension_data,'|',1) as text) ) as batchunique,a.rate, 
				cast(SPLIT_PART(a.dimension_data,'|',2) as numeric) as qty, 
				b.supplier as mg_supplierid, b.docid, b.location, 'bcanc' as transid, a.grn_itemsrow
			from (select unnest( string_to_array( cast( new.batchserialbreakup as text),',')) as dimension_data, new.grn_itemsid as grn_itemsid, 
					new.stockrate rate, new.itemname as itemname, new.grn_headerid as grn_headerid, new.grn_itemsrow as grn_itemsrow
					where new.invmethod::text != '1' 
					) a, grn_header b where a.grn_headerid = b.grn_headerid
		)
		ON CONFLICT (company, sourceid, itemname, batch)
		DO UPDATE SET batch = EXCLUDED.batch, 
              rate = EXCLUDED.rate, 
              itemname = EXCLUDED.itemname,
			  qty = EXCLUDED.qty,
			  docid = EXCLUDED.docid,
			  mg_supplierid = EXCLUDED.mg_supplierid,
			  modifiedon = EXCLUDED.modifiedon,
			  username = EXCLUDED.username,
			  cancel = EXCLUDED.cancel,
			  location = EXCLUDED.location,
			  expiry_date = EXCLUDED.expiry_date,
			  grn_itemsrow = excluded.grn_itemsrow;			

--		if TG_OP = 'UPDATE' then 
		
			update batchandcost a set qty = 0  where a.sourceid = new.grn_itemsid and not exists
			(select 'x'
			from 
			(
				select cast(SPLIT_PART(a.dimension_data,'|',1) as text) as batch, a.grn_itemsid, a.itemname
				from 
				(
				select unnest( string_to_array( cast( new.batchserialbreakup as text),',')) as dimension_data, new.grn_itemsid as grn_itemsid, 
				new.stockrate costrate, new.itemname as itemname, new.grn_headerid as grn_headerid
				) a
			) b 
			where --a.sourceid = b.grn_itemsid 
			--and 
			a.batch = b.batch 
...
--		END IF;
		...
		delete from batchandcost where sourceid = old.grn_itemsid;
	end if;						 						 		  			 						  
	RETURN NEW;
...
CREATE FUNCTION {schema}.func_pricepickerv2(pitemid numeric, pcompany numeric, pcustomer text, pbranch text, pdocdate date DEFAULT CURRENT_DATE, pprimaryprice numeric DEFAULT 2, pcurrency character varying DEFAULT 'INR'::character varying) RETURNS numeric
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
    result_price numeric :=	-1 ;
	mfromdate 	 date 	 := current_date;
BEGIN
/*
		-- FIRST CHECK IF ANY PRICE EXISTS AGAINST CUSTOMER
		select coalesce(max(c.primarysellingrate),0) into result_price
		from (

		select b.by_customer,
		unnest( string_to_array( cast( b.branch_customerdata as text),',')) as dimension_data, b.fromdt, 
		case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate
		FROM sellingprice b
		where b.active  = 'T' and b.fromdt = (
					select max(a.fromdt) as fromdate
					from sellingprice a 
					where a.fromdt  <= current_date
					and a.cancel = 'F' 
					and a.itemname = b.itemname 
					and a.by_customer = b.by_customer
					and a.company = b.company
					and a.active  = 'T'
					and a.dataselection = b.dataselection
					and a.currency = b.currency
					)
		and b.fromdt  <= current_date
		and b.cancel = 'F' 
		and b.itemname = pitemid::numeric
		and b.by_customer = 'T'
		and b.company = pcompany::numeric
		and b.active  = 'T'
		and b.dataselection = 'Selected'
		and b.currency = pcurrency

		) c
		where c.dimension_data = pcustomer;
		
		if result_price = 0 or result_price is null then 

			-- 2ND CHECK IF ANY PRICE EXISTS AGAINST BRANCH
			
			select coalesce(max(c.primarysellingrate),0) into result_price
			from (
			select b.by_branch, unnest( string_to_array( cast( b.branch_customerdata as text),',')) as dimension_data, b.fromdt, 
			case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate  
			FROM sellingprice b
			where b.active  = 'T' and b.fromdt = (
						select max(a.fromdt) as fromdate
			...
			where c.dimension_data = pbranch;

			if result_price = 0 or result_price is null then 

				-- 3RD CHECK IF ANY PRICE EXISTS AGAINST CUSTOMER OR BRANCH WITH ALL OPTION
		
				select max(a.fromdt) into mfromdate
				from sellingprice a 
				where a.fromdt  <= pdocdate
				and a.cancel = 'F' 
				and a.itemname = pitemid::numeric 
				and ( a.by_customer = 'T' or a.by_branch = 'T')
				and a.company = pcompany::numeric
				and a.active  = 'T'
				and a.dataselection = 'ALL'
				and a.currency = pcurrency;
				select max(
				case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end 
				) into result_price
				FROM sellingprice b
				where b.active  = 'T' 
				and b.fromdt = mfromdate
				and b.cancel = 'F' 
				and b.itemname = pitemid::numeric 
				and ( b.by_customer = 'T' or b.by_branch = 'T')
				and b.company = pcompany::numeric
				and b.active  = 'T'
				and b.dataselection = 'ALL'
				and b.currency = pcurrency;	
				if 	result_price = 0  or result_price is null then 
					-- 4TH CHECK IF ANY PRICE EXISTS AGAINST ITEM MASTER
					select stdsellingprice into result_price from item a where a.company = 	pcompany::numeric and a.itemid = pitemid::numeric ;
				end if;		
			end if;
		end if;
*/
	select coalesce(primarysellingrate,0) into result_price
	from (
	select 1 as id, case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate 
	FROM sellingprice b
	where b.by_customer = 'T' 
	and b.by_branch = 'F'
	and b.company = pcompany::numeric
	and b.fromdt  <= current_date
	and b.cancel = 'F'
	and b.app_desc = 1
	and b.selected_customers like '%' || pcustomer || '%' 
	and b.active = 'T'
	and b.itemname = pitemid::numeric
	and b.fromdt = (
		select max(a.fromdt) as fromdate
		from sellingprice a 
		where a.by_customer = b.by_customer 
		and	a.fromdt  <= current_date
		and a.cancel = 'F' 
		and a.itemname = b.itemname 
		and a.by_branch = b.by_branch
		and a.company = b.company
		and a.active  = 'T'
		and a.selected_customers = b.selected_customers
		and a.currency = b.currency	)
	union all
	select 2 as id, case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate 
	FROM sellingprice b
	where b.by_customer = 'T' 
	and b.by_branch  = 'T'
	and b.company = pcompany::numeric
	...
	and b.app_desc = 1
	and b.selected_customers like '%' || pcustomer || '%'
	and b.selected_branches like '%' || pbranch || '%' 
	and b.active = 'T'
	and b.itemname = pitemid::numeric
	and b.fromdt = (
		select max(a.fromdt) as fromdate
		...
		and a.active  = 'T'
		and a.selected_customers = b.selected_customers
		and a.selected_branches = b.selected_branches
		and a.currency = b.currency	)
	union all 
	select 3 as id, case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate 
	FROM sellingprice b
	where b.by_customer = 'F' 
	and b.by_branch  = 'T'
	and b.company = pcompany::numeric
	and b.fromdt  <= current_date
	and b.cancel = 'F'
	...
	and b.fromdt = (
		...
		and a.currency = b.currency	) 
	union all
	select 4 as id, case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate 
	FROM sellingprice b
	where b.by_customer = 'F' 
	and b.by_branch  = 'F'
	and b.company = pcompany::numeric
	...
	union all
	select 5 as id, case when pprimaryprice = 2 then b.sellingunitsellingprice else b.primarysellingrate end as primarysellingrate 	 
	from sellingprice b where b.itemname = pitemid::numeric
	) a 
	order by id limit 1;
...
CREATE FUNCTION {schema}.func_returns_hyperlink(preference character varying, phreforclass character varying DEFAULT 'class'::character varying, ptransid character varying DEFAULT ' '::character varying, precordid numeric DEFAULT 0, pcaption character varying DEFAULT ' '::character varying, mpreference character varying DEFAULT ' '::character varying) RETURNS text
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
...
begin
    if pHREForClass = 'class' then
        mHyperLink = '<div><a role="button" class="menu-link"'||
                    replace('o-n-c-l-i-c-k' :: text,'-','')
                    ||'="parent.LoadIframe(''htmlPages.aspx?loadcaption=AxTaskManager&taskid='||pReference||''')"><span class="menu-icon"><i class="setting-icon material-icons">task
            </i></span></a></div>';
    elseif pHREForClass = 'formpage1'  then -- void checking - just to know the original syntax
    	        mHyperLink = '<div><a role="button" class="menu-link"'||
                    replace('o-n-c-l-i-c-k' :: text,'-','')
--                    ||'="parent.LoadIframe(''EntityForm.aspx?ename='||pcaption||'&tstid='||ptransid||'&recid='||precordid::text||'&keyval='||pReference||''')"><span class="menu-icon"><i class="setting-icon material-icons">task</i></span></a></div>';           
               ||'="parent.ShowDimmer(true);parent.LoadIframe(''EntityForm.aspx?ename='||pcaption||'&tstid='||ptransid||'&recid='||precordid::text||'&keyval='||mPreference||''')">'||pReference||'</a></div>';            
           
-- SAMPLE           <div><a class="menulink" onclick ="parent.LoadIframe('EntityForm.aspx?ename=Request&tstid=transA&recid=recid1&keyval=DOCA')">DOCA</a></div>
    elseif pHREForClass = 'formpage'  then
    	        mHyperLink = '<div><a role="button" class="menu-link"'||
                    replace('o-n-c-l-i-c-k' :: text,'-','')
--                    ||'="parent.LoadIframe(''EntityForm.aspx?ename='||pcaption||'&tstid='||ptransid||'&recid='||precordid::text||'&keyval='||pReference||''')"><span class="menu-icon"><i class="setting-icon material-icons">task</i></span></a></div>';           
               ||'="parent.ShowDimmer(true);parent.LoadIframe(''EntityForm.aspx?tstid='||ptransid||'&recid='||precordid::text||''')">'||pReference||'</a></div>';            
           
-- SAMPLE           <div><a class="menulink" onclick ="parent.LoadIframe('EntityForm.aspx?ename=Request&tstid=transA&recid=recid1&keyval=DOCA')">DOCA</a></div>           
	else            
        mHyperLink = '<div><a role="button" href="#" class="menu-link" '||
                    replace('o-n-c-l-i-c-k' :: text,'-','')
...
CREATE FUNCTION {schema}.func_stockrecalculation_costs(pcompany numeric, pbranch character varying DEFAULT 'ALL'::character varying, pdocdate date DEFAULT CURRENT_DATE) RETURNS void
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
    ...
    mi RECORD;
    mt RECORD;
    mcumulative_qty numeric:=0;
    mcumulative_amount numeric:=0;
	mrate numeric:=0;
	mbranchid numeric :=0;
BEGIN
    -- Sort outward transactions by date

	select branchid into mbranchid from branch a where a.branchname = pbranch and a.companyname = pcompany;

	update batchmaster set wavgvalue =0 where branch = mbranchid;

	update stockvalue set allocated_qty =0,  wavg_costrate = 0 where branch = mbranchid;
	
	update stockvalue set wavg_costrate = rate where lower(plusorminus) = 'p' and branch = mbranchid;

    FOR mb IN  (SELECT branchid, branchname FROM branch a where a.companyname = pcompany and a.cancel ='F' and a.active = 'T' and a.branchid = mbranchid order by 2)
    LOOP
		FOR mi IN  (SELECT itemid, itemname, invmethod FROM item a where a.company = pcompany and a.cancel ='F' and a.active = 'T'  order by 2)
	    LOOP
			mcumulative_qty =0;
		    mcumulative_amount =0;
			mrate =0;

			FOR mt IN  (SELECT a.stockvalueid, a.itemname, a.qty, a.stock_qty, a.rate, a.amount, a.plusorminus, 
									b.modifiedon, b.createdon
									 FROM stockvalue a, stockvalueheader b
									where 
									      a.stockvalueheaderid = b.stockvalueheaderid
									  and a.company = pcompany 
  									  and b.cancel = 'F'
									  and a.branch = mb.branchid 
                                      and a.itemname = mi.itemid
									  and a.docdate <= pdocdate
									  order by a.docdate, b.createdon, a.plusorminus desc)
		    LOOP

				if lower(mt.plusorminus) = 'm' then
					update stockvalue a set wavg_costrate = mrate, amount = a.stock_qty * mrate, last_updated = now() where a.stockvalueid = mt.stockvalueid;
				else 
					update stockvalue a set wavg_costrate = mt.rate, last_updated = now() where a.stockvalueid = mt.stockvalueid;
				end if;	
	
				mcumulative_qty = mcumulative_qty + mt.stock_qty;
				mcumulative_amount = mcumulative_amount + case when lower(mt.plusorminus) = 'p' then mt.amount else mt.stock_qty * mrate end;
				mrate = case when mcumulative_qty != 0 then mcumulative_amount / mcumulative_qty else 0 end;
...
CREATE FUNCTION {schema}.func_stockrecalculation_fifocosts(pcompany numeric, pbranch numeric DEFAULT 0, pdocdate date DEFAULT CURRENT_DATE, pitemid numeric DEFAULT 0) RETURNS void
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
-- ALTERED ON 08 JUNE 2026 - TO BE TESTED 
-- PRIOR VERSION IS AVAILABLE IN func_stockrecalc_fifocosts_trial
DECLARE
    mb RECORD;
    mi RECORD;
    mplus RECORD;
	mminus RECORD;
	remaining_qty numeric :=0;
	mbranchid numeric := 0;
	mfifo_stockvalue numeric :=0;
	mallocatedqty numeric :=0;
	msourcedocs varchar := '';
BEGIN

--	select branchid into mbranchid from branch a where a.branchname = pbranch and a.companyname = pcompany;

--	update batchmaster set fifovalue =0 where branch = mbranchid;

--select * from stockvalue a

--	update stockvalue set sourcedocs = ' ' where lower(plusorminus) = 'm' and branch = mbranchid;

--	delete from temp;

    -- Sort outward transactions by date
    FOR mb IN  (SELECT branchid, branchname FROM branch a where a.companyname = pcompany and a.cancel ='F' and a.active = 'T' and 
					(a.branchid = pbranch or pbranch = 0) order by 2)
    LOOP
--		FOR mi IN  (SELECT itemid, itemname, invmethod FROM item a where a.company = pcompany and a.cancel ='F' and a.active = 'T'  
--							and (a.itemid = pitemid or pitemid = 0) order by 2)
		FOR mi in (select a.branch as branchid, a.itemname as itemid, min(a.last_modifieddate) as lastmodified from stockvalue_summary a where a.branch = mb.branchid
				and a.cancel = 'F' and a.app_desc = 1  
				group by a.branch, a.itemname
				order by 1,2) 
	    LOOP

			update stockvalue a set allocated_qty =0, calculated_costrate = 0, calculated_stockvalue = 0
							  where branch = mi.branchid and itemname = mi.itemid 
									and lower(plusorminus) = 'm'
									and a.cancel = 'F'
									and a.docdate >= mi.lastmodified;	

			update stockvalue a set calculated_costrate = rate
								where lower(plusorminus) = 'p' 
								and branch = mi.branchid 
								and itemname = mi.itemid 
								and a.docdate >= mi.lastmodified
								and a.cancel = 'F';

			--update stockvalue set sourcedocs = ' ' where lower(plusorminus) = 'm' and branch = mi.branchid and mi.itemname = mi.itemid;

			FOR mminus IN  (SELECT 	a.stockvalueid, a.itemname, a.qty, a.stock_qty + a.allocated_qty as stock_qty, a.rate, a.amount, a.plusorminus, 
									a.modifiedon, a.createdon, a.docdate, a.allocated_qty, a.docid, a.allocated_qty * a.calculated_costrate as fifostockvalue, 
									coalesce(a.sourcedocs,' ') sourcedocs
									FROM stockvalue a
									where a.company = pcompany 
  									  and a.cancel = 'F' 
									  and lower(a.plusorminus) = 'm'
									  and a.branch = mi.branchid 
                                      and a.itemname = mi.itemid
									  and a.docdate >= mi.lastmodified
									  and (a.stock_qty + a.allocated_qty) < 0 								
									  order by a.docdate, a.createdon)
				LOOP

					remaining_qty = abs(mminus.stock_qty);
					mfifo_stockvalue = mminus.fifostockvalue;
					mallocatedqty = mminus.allocated_qty;
					msourcedocs = mminus.sourcedocs;
		
					if msourcedocs <> ' ' or msourcedocs is not null then
--					raise notice '%',msourcedocs;
					update stockvalue s set allocated_qty = s.allocated_qty + a.allocated_qty 
					from (select cast(SPLIT_PART(unnest( string_to_array( cast( substr(trim(msourcedocs),2,length(msourcedocs)) as text),',')),'|',1) as varchar) as docid,cast( REGEXP_REPLACE( cast(SPLIT_PART(unnest( string_to_array( cast( substr(trim(msourcedocs),2,length(msourcedocs)) as text),',')),'|',2) as varchar),'[^0-9.]', '', 'g') as numeric) as allocated_qty ) a
					 where trim(a.docid) = s.docid and s.itemname = mminus.itemname and lower(s.plusorminus) = 'p' ;
					end if;	

					msourcedocs = ' ';

					FOR mplus IN  (SELECT a.stockvalueid, a.itemname, a.qty, a.stock_qty + a.allocated_qty as stock_qty, a.rate, a.amount, a.plusorminus, 
									a.modifiedon, a.createdon, a.allocated_qty, a.docid
									 FROM stockvalue a 
									where a.cancel = 'F'
									  and lower(a.plusorminus) = 'p'
									  and a.branch = mi.branchid 
                                      and a.itemname = mi.itemid
									  and a.docdate <= mminus.docdate 
									  and (a.stock_qty + a.allocated_qty) > 0 
									  order by a.docdate, a.createdon)
				    LOOP				
						...
						if mplus.stock_qty >= remaining_qty  then 
							
							update stockvalue a set allocated_qty = allocated_qty - remaining_qty where stockvalueid = mplus.stockvalueid; 
							--last_updated = now() 

							mfifo_stockvalue = mfifo_stockvalue + (remaining_qty * mplus.rate);
							mallocatedqty = mallocatedqty + remaining_qty;							

							remaining_qty = 0;

						else 			

							remaining_qty = mplus.stock_qty - remaining_qty;
							
							update stockvalue a set allocated_qty = allocated_qty - mplus.stock_qty
 where stockvalueid = mplus.stockvalueid;

							mfifo_stockvalue = mfifo_stockvalue + (mplus.stock_qty * mplus.rate);
							mallocatedqty = mallocatedqty + mplus.stock_qty;

							remaining_qty = abs(remaining_qty);

						end if;

/*						if mminus.plusorminus = 'm' then 

							insert into temp (itemname, outdocid,fifo_stockvalue, allocated_qty) (select mminus.itemname, mminus.docid, mfifo_stockvalue, mallocatedqty);

						end if;
*/
 
						msourcedocs = msourcedocs|| ',' || mplus.docid || '|' || mallocatedqty ;
					    mallocatedqty = 0;


					END LOOP; -- PLUS LOOP
						update stockvalue a set allocated_qty = qty - remaining_qty, 
--											last_updated = now(), 
											calculated_costrate = case when (qty - remaining_qty) > 0 then mfifo_stockvalue / (qty - remaining_qty) else 0 end, 
											sourcedocs = msourcedocs
								where a.stockvalueid = mminus.stockvalueid;
		...
		update stockvalue_summary a set last_modifieddate = now(), updatelog = 1 where a.itemname = mi.itemid and a.branch = mi.branchid;

	END LOOP; -- BRANCH LOOP

END;
$$;

CREATE FUNCTION {schema}.func_stockrecalculation_fifocosts(pcompany numeric, pbranch character varying DEFAULT 'ALL'::character varying, pdocdate date DEFAULT CURRENT_DATE, pitemid numeric DEFAULT 0, pclosingstock numeric DEFAULT 0, pposttostock character varying DEFAULT 'F'::character varying) RETURNS void
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
-- ALTERED ON 08 JUNE 2026 - TO BE TESTED 
...
--create table temp (mydata varchar(200))
...
--	delete from temp;
    ...
    -- Sort outward transactions by date
    FOR mb IN  (SELECT branchid, branchname FROM branch a where a.companyname = pcompany and a.cancel ='F' and a.active = 'T' and 
					(a.branchname = pbranch or pbranch = 'ALL') order by 2)
...
--1040990000000
--			insert into temp (mydata) values ( concat( pcompany,' ',pbranch,' ',now())) ;
	    ...
	    LOOP
			...
			update stockvalue a set allocated_qty =0, calculated_costrate = 0, calculated_stockvalue = 0, sourcedocs = ' '
								where lower(plusorminus) = 'm'									 
									and branch = mi.branchid 
									and itemname = mi.itemid
									and a.docdate >= mi.lastmodified
									and a.cancel = 'F';	

			update stockvalue a set calculated_costrate = rate, calculated_stockvalue = rate * qty, allocated_qty = 0 
								where lower(plusorminus) = 'p' 
								and branch = mi.branchid 
								and itemname = mi.itemid 
								and a.docdate >= mi.lastmodified
...
/*SELECT 	a.stockvalueid, a.itemname, a.qty, a.stock_qty + a.allocated_qty as stock_qty, a.rate, a.amount, a.plusorminus, 
									a.modifiedon, a.createdon, a.docdate, a.allocated_qty, a.docid, a.allocated_qty * a.calculated_costrate as fifostockvalue, 
									coalesce(a.sourcedocs,' ') sourcedocs
									FROM stockvalue a
									where a.company = :pcompany 
...
--                                      and a.itemname = :itemid
									  and a.docdate >= :lastmodified
									  and (a.stock_qty + a.allocated_qty) < 0 								
									  order by a.docdate, a.createdon
*/
				...
				LOOP
					...
					FOR mplus IN  (
/*									SELECT a.stockvalueid, a.itemname, a.qty, a.stock_qty + a.allocated_qty as stock_qty, a.rate, a.amount, a.plusorminus, 
									a.modifiedon, a.createdon, a.allocated_qty, a.docid, 2 as ord, a.docdate
									 FROM stockvalue a 
									where a.cancel = 'F'
									  and lower(a.plusorminus) = 'p'
									  and a.branch = mi.branchid 
                                      and a.itemname = mi.itemid
									  and a.docdate between mminus.docdate and mi.lastmodified
									  and (a.stock_qty + a.allocated_qty) > 0
									union 
*/								
									SELECT a.stockvalueid, a.itemname, a.qty, a.stock_qty + a.allocated_qty as stock_qty, a.calculated_costrate, 
									a.stock_qty * a.calculated_costrate as amount, a.plusorminus, 
									a.modifiedon, a.createdon, a.allocated_qty, a.docid, 1 as ord, a.docdate, a.rate
									 FROM stockvalue a 
									where a.cancel = 'F'
									  and lower(a.plusorminus) = 'p'
									  and a.branch = mi.branchid 
...
--									  and a.docdate < mi.lastmodified 
				    ...
				    LOOP				
						...
						if mplus.stock_qty >= remaining_qty  then 
							...
							update stockvalue a set allocated_qty = allocated_qty - mplus.stock_qty where stockvalueid = mplus.stockvalueid;
...
*/
						...
						msourcedocs = msourcedocs|| ',' || mplus.docid ;
...
--											last_updated = now(), 
											calculated_costrate = case when (qty - remaining_qty) > 0 then mfifo_stockvalue / (qty - remaining_qty) else 0 end, 
											calculated_stockvalue = case when (qty - remaining_qty) > 0 then mfifo_stockvalue / (qty - remaining_qty) else 0 end * a.stock_qty,
											sourcedocs = msourcedocs
								where a.stockvalueid = mminus.stockvalueid;
			...
			update stockvalue_summary a set last_modifieddate = now(), costrate = case when a.inward - a.outward >0 then mfifo_stockvalue /a.inward - a.outward else 0 end
				where a.itemname = mi.itemid and a.branch = mi.branchid;
...
CREATE FUNCTION {schema}.func_stockrevaluation() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/* ALTERED ON 09 APRIL 2025 BY VAIDHEESH*/ 
declare 
	mbranches varchar :='';
	results varchar := '';
begin		 
	
/*	if POSITION('weighted average' IN lower(new.valuationmethod) ) != 0 then

		FOREACH mbranches IN ARRAY string_to_array(new.branch, ',')
	    LOOP
--			RAISE NOTICE 'Weighted Average is being processed for Branch: %', mbranches;
			select func_stockrecalculation_costs( new.company, mbranches, new.docdate ) into results;
	    END LOOP;

	end if;

	if POSITION('fifo' IN lower(new.valuationmethod) ) != 0 then 
		FOREACH mbranches IN ARRAY string_to_array(new.branch, ',')

	    LOOP
--			RAISE NOTICE 'FIFO is being processed for Branch: %', mbranches;
*/
--			select func_stockrecalculation_fifocosts( new.company, mbranches, new.docdate ) into results;
		PERFORM func_stockrecalculation_fifocosts( new.company, new.branch, new.docdate ) ;

--		PERFORM func_accvouchercreationupdate();
/*  
	    END LOOP;
	end if;
*/
...
CREATE FUNCTION {schema}.func_stockvaluesummaryinward() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
    msourceid NUMERIC := 0;
    mitemdesc VARCHAR(255) := '';  
    h RECORD;
	mdocdate date;
	mndocdate date;
    /*Created by Durga on 23/06/2025 29/09/2025*/
BEGIN
 
    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
		select last_modifieddate into mdocdate from stockvalue_summary WHERE location = NEW.location AND itemname = NEW.itemname;
 
        SELECT * INTO h FROM stockvalue WHERE stockvalueid = NEW.stockvalueid;
 
        INSERT INTO stockvalue_summary (
            stockvalue_summaryid, cancel, sourceid, username, modifiedon, createdby, createdon,
            wkid, app_level, app_desc, app_slevel, cancelremarks, wfroles,
            company, branch, location, itemname, inward, outward, reserved,last_modifieddate,temp_reserved, updatelog
        )
        VALUES (
            nextval('inbound_inboundid_seq'), h.cancel, 0, h.username, h.modifiedon, h.createdby, h.createdon,
            h.wkid, h.app_level, h.app_desc, h.app_slevel, h.cancelremarks, h.wfroles,
            NEW.company, NEW.branch, NEW.location, NEW.itemname, NEW.qty, 0, 0,NEW.docdate,0,0
        )
        ON CONFLICT (company, branch, location, itemname)
        DO UPDATE SET  
            itemname = EXCLUDED.itemname,
            inward = stockvalue_summary.inward + EXCLUDED.inward,
            modifiedon = EXCLUDED.modifiedon,
            username = EXCLUDED.username,
            cancel = EXCLUDED.cancel,
            location = EXCLUDED.location,
			last_modifieddate = LEAST(NEW.docdate,mdocdate);
    END IF;
 
	
    -- Update inward only if qty/location/itemname has changed
    IF TG_OP = 'UPDATE' THEN
		select last_modifieddate into mndocdate from stockvalue_summary WHERE location = OLD.location AND itemname = OLD.itemname;
		
        UPDATE stockvalue_summary
        SET inward = inward - OLD.qty,last_modifieddate = LEAST(NEW.docdate,mndocdate), updatelog = 0
        WHERE location = OLD.location AND itemname = OLD.itemname;
    END IF;
    ...
    IF TG_OP = 'DELETE' THEN
        ...
        UPDATE stockvalue_summary
        SET inward = inward - OLD.qty,
		last_modifieddate = LEAST(old.docdate,mndocdate), 
		updatelog = 0
        WHERE location = OLD.location AND itemname = OLD.itemname;
...
EXCEPTION
    WHEN check_violation THEN
        RAISE EXCEPTION 'Stock not available: Currently allocated to Orders. %', mitemdesc;
END;
$$;

CREATE FUNCTION {schema}.func_stockvaluesummaryoutward() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
	...
	mndocdate DATE;
BEGIN
	SELECT last_modifieddate INTO mdocdate from stockvalue_summary WHERE itemname = NEW.itemname AND location = NEW.location;
 
	SELECT last_modifieddate INTO mndocdate from stockvalue_summary WHERE itemname = OLD.itemname AND location = OLD.location;
 
    IF TG_OP = 'INSERT' THEN
 
        UPDATE stockvalue_summary
        SET outward = outward + COALESCE(NEW.qty, 0),
		last_modifieddate = LEAST(mdocdate,NEW.docdate), 
		updatelog = 0
    ...
    ELSIF TG_OP = 'UPDATE'  THEN
        ...
        UPDATE stockvalue_summary
        SET outward = outward - COALESCE(OLD.qty, 0),
		last_modifieddate = LEAST(mndocdate,NEW.docdate), 
		updatelog = 0
        WHERE location = OLD.location
          AND itemname = OLD.itemname;
    ...
    ELSIF TG_OP = 'DELETE' THEN
        ...
        UPDATE stockvalue_summary
        SET outward = outward - COALESCE(OLD.qty, 0),
		last_modifieddate = LEAST(mndocdate,old.docdate), 
		updatelog = 0
        WHERE location = OLD.location
          AND itemname = OLD.itemname;
    END IF;
...
CREATE FUNCTION {schema}.func_update_arapadjustments() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
declare 
   piscomboitem varchar(1) := 'F';
   pqty numeric(13,3) := 0;
   mqty numeric:= 1;						
   mflag numeric:= 1;
   mtotalqty numeric:= 0;
...
begin		
	...
	case when TG_OP = 'INSERT' then 
			update arapdetails set settled = settled + new.amount, bsettled = bsettled + new.bamount, nsettled = nsettled + new.namount
			where company = new.company and subledgercode = new.subledgername and docid = new.refno;
		when TG_OP = 'UPDATE' then 
			update arapdetails set settled = settled + new.amount - old.amount , bsettled = bsettled + new.bamount - old.bamount, nsettled = nsettled + new.namount - old.namount
			where company = new.company and subledgercode = new.subledgername and docid = new.refno;
		when  TG_OP = 'DELETE' then 
			update arapdetails set settled = settled - old.amount , bsettled = bsettled - old.bamount, nsettled = nsettled - old.namount			
			where company = old.company and subledgercode = old.subledgername and docid = old.refno;		
	end case;
/*
	if not found then  
		RAISE EXCEPTION 'Adjustment Source transaction is not found';				        								    		
	end if;			 						    
*/
	RETURN NEW;
...
CREATE FUNCTION {schema}.function_copycoafromtemplate() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
declare 
...
begin
	...
	select currencyid into mcurrencyid from currency where currency = new.gcurrency;

    
	if TG_OP = 'UPDATE'  then 	

		update axpdef_company set company_name = new.companyname, company_country = new.country, company_state = new.state, company_address = new.companyaddress, 
													 company_regno = new.taxcode, company_displaytitle = new.companyname, cancelremarks =  'UPDATED FROM COMPANY MASTER'
													where axpdef_companyid = new.companyid;												
		if not found then 			
				INSERT INTO axpdef_company 
				(axpdef_companyid, cancel, sourceid, mapname, username, modifiedon, createdby, createdon, wkid, app_level, app_desc, app_slevel, cancelremarks, wfroles, 
				company_name, company_country, company_state, company_address, company_regno, company_displaytitle, primarycompany)
					(select new.companyid axpdef_companyid, new.cancel, new.sourceid, new.mapname, new.username, new.modifiedon, 
					new.createdby, new.createdon, new.wkid, new.app_level, new.app_desc, new.app_slevel, 
					'UPDATED FROM COMPANY MASTER' cancelremarks, new.wfroles, 
					new.companyname company_name, new.country company_country, new.state company_state, new.companyaddress company_address, 
					new.taxcode company_regno, new.companyname company_displaytitle, 'F' primarycompany
					from dual);
		end if;				

	end if;

	if ( TG_OP = 'INSERT' and new.active = 'T') then 	
		
		INSERT INTO mg_account
			(mg_accountid, cancel, sourceid, mapname, username, 
			modifiedon, createdby, createdon, wkid, app_level, app_desc, app_slevel, cancelremarks, wfroles, transid, 
			atype, primarygroup, alie, groupname, glevel, alevel, prefix, 
			accountcode, accountname, acategory, acatcode, acurrency, 
			hbranch,  
			ibaccount, controlaccount, budgetctrl, budgetflag, active,  
			groupcode, company, company_code, group_accountcodes, addable, company_accountname, accountcode_alias, 
			default_group, common_intergration_code, applicablebranches)
			(
			select 
			nextval( 'inbound_inboundid_seq' ) as mg_accountid , a.cancel, new.companyid sourceid, null mapname, new.username username, 
			new.modifiedon modifiedon, new.createdby as createdby , new.createdon createdon, new.wkid, a.app_level app_level, a.app_desc, 
			a.app_slevel, a.cancelremarks, a.wfroles, 'fiacc' transid, 
			a.atype, a.primarygroup, a.alie, a.groupname, a.glevel, a.alevel, new.company_code || substring(a.prefix,3,2) prefix, 
			new.company_code || substring(a.accountcode,3,50) as accountcode,  a.accountname, a.acategory, a.acatcode, mcurrencyid, 
			null hbranch,  
			a.ibaccount, a.controlaccount, a.budgetctrl, a.budgetflag, a.active, 
			new.company_code || substring(a.groupcode,3,50) as groupcode, new.companyid as company, 
			new.company_code as company_code, 
			new.company_code || substring(a.group_accountcodes,3,50) group_accountcodes, addable, 
			new.company_code || substr(company_accountname,3,500) company_accountname, accountcode_alias, 
			a.default_group, a.common_intergration_code, 'ALL' as applicablebranches
			from mg_account_template a, mg_coamastertemplate b 
			where b.template_code = a.company_code
			and b.template_desc = new.coa_template
			);
		
			insert into sequence  
			(sequenceid, prefix, transtype, fieldname, activesequence, description, lastno, corderby, optionname, prefixfield, noofdigits)			
			(select nextval( 'inbound_inboundid_seq' ) as sequenceid, 
			new.company_code||substr(prefix,2,5) as prefix , 
			a.transtype, a.fieldname, a.activesequence, a.description, a.lastno, a.corderby, a.optionname, a.prefixfield, a.noofdigits
			from sequence_template a);
	
			if upper(new.taxtype) = 'GST' then		 
				INSERT INTO taxtypes
				(taxtypesid, cancel, sourceid, mapname, username, modifiedon, createdby, createdon, wkid, app_level, app_desc, app_slevel, cancelremarks, wfroles, taxcode, taxdescription, 
				printsequence, ismandatory, active, company, formula, sgstoutputtax, cgstoutputtax, igstoutputtax, sgstinputtax, cgstinputtax, igstinputtax, dupcheck)
				(select nextval( 'inbound_inboundid_seq' ), 'F', 0, NULL, NEW.username, new.modifiedon, 
				new.createdby, new.createdon, NULL, 1, 1, NULL, NULL, NULL, 'GST', 'GST', 0, 'F', 'T', new.companyid, 
				'basicamount*rate*0.01', 'SGST Output', 'CGST Output', 'IGST Output', 'SGST Input', 'CGST Input', 'IGST Input', new.companyid::text||'GST'::text );
			end if;
										
	end if;
	
	if TG_OP = 'DELETE'  then
		delete from axpdef_company a where axpdef_companyid = new.companyid;	
	end if;
			
/* 
	IF ( TG_OP != 'INSERT' or new.active = 'F')  then
      
        DELETE FROM axattachworkflow WHERE wkid = old.pwkid;
       
        DELETE FROM axworkflow WHERE name = old.pwkid;
       
--        RETURN OLD;
    END IF;
 
    IF TG_OP != 'DELETE' and new.active = 'T' then
    
    		new.xml_fulltext := replace( new.xml_fulltext, '>,','>');
       
       		insert into axworkflow (name, caption, props,wno) values
       		( new.pwkid, new.workflow_description, new.xml_fulltext, 1 );
       	
       		insert into axattachworkflow (wkid, transid, wf_condition, condtext, wf_edit, wf_cancel, wf_reapproval ) values
       		( new.pwkid, new.ptransid, new.pcondition, new.pcondition, new.allowedit, new.allowcancel, new.allowreapproval);
       	              	       	
	       	if coalesce(to_regclass( trim(new.ptransid)||'workflow' ),'1') = '1' then
       	       	
	       		execute 'CREATE TABLE '||trim(new.ptransid)||'workflow (
					recordid numeric(16) NULL,
					"comments" varchar(250) NULL,
					updusername varchar(50) NULL,
					app_datetime varchar(25) NULL,
...
CREATE FUNCTION {schema}.function_udfworkflow() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
...
--        RETURN OLD;
    ...
    IF TG_OP != 'DELETE' and new.active = 'T' then    
    		new.xml_fulltext := replace( new.xml_fulltext, '>,','>');       
       		insert into axworkflow (name, caption, props,wno, appemail, mailcontent, chkms, url, attach, auth, chkrem, cc, bcc, mailsub ) values
       		( new.pwkid, new.workflow_description, new.xml_fulltext, 1, new.allowemailapproval, new.emailbody, new.emailbyoutlook, new.emailurl, new.emailattachment,
			  new.emailauthentication, 'T', new.emailcc, new.emailbcc, new.emailsubject);       	
       		insert into axattachworkflow (wkid, transid, wf_condition, condtext, wf_edit, wf_cancel, wf_reapproval ) values
       		( new.pwkid, new.ptransid, new.pcondition, new.pcondition, new.allowedit, new.allowcancel, new.allowreapproval);       	              	       	
	       	if coalesce(to_regclass( trim(new.ptransid)||'workflow' ),'1') = '1' then       	       	
	       		execute 'CREATE TABLE '||trim(new.ptransid)||'workflow (
...
CREATE FUNCTION {schema}.function_updatedenormalized() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
/*declare 
...
begin
	...
	if TG_OP = 'UPDATE' and (new.acatcode = 's' or new.alie in ('3','4')) and new.atype = 'Account' and old.accountname != new.accountname then

		if new.alie= '3' then
			update itemaccountmapping set saleslaccount = new.accountname where salesacid = new.mg_accountid;
			update item set salesac = new.accountname where salesacid = new.mg_accountid;
		end if;

		if new.alie = '4' then 
			update itemaccountmapping set purchaseaccount = new.accountname where purchaseacid = new.mg_accountid;
			update item set purchaseac = new.accountname where purchaseacid = new.mg_accountid;
		end if;

		if new.acatcode = 's' then
			update itemaccountmapping set stockaccount = new.accountname where stockacid = new.mg_accountid;
			update item set stockac = new.accountname where stockacid = new.mg_accountid;
		...
		if TG_OP = 'UPDATE' then 
			...
			update costcentre_accountmapping set accountname = new.accountname where mg_accountid = new.mg_accountid;
...
/* 
	if TG_OP = 'DELETE' and old.atype = 'Account' and (old.acatcode = 's' or old.alie in ('3','4')) then

		-- NEEDS TO BE RESTRICTED FROM DELETING	
...
CREATE FUNCTION {schema}.get_exrate(p_company numeric, p_currency numeric, p_sellingrate character DEFAULT 'T'::bpchar, p_asondate timestamp with time zone DEFAULT CURRENT_DATE) RETURNS numeric
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
DECLARE
    ...
    maxdate  timestamp;
begin
    select basecurrency into bcurr from company where companyid = p_company; 
    select max(modifiedon) into maxdate from mg_exrates where currency = p_currency
	--and docdate<=p_asondate
	and company = p_company;    
    --raise_application_error(-20005,'p_sellingrate'||p_sellingrate);    
    if p_currency=bcurr then
        l_result := 1;
    end if;
    if p_currency<>bcurr then
        select CASE WHEN p_sellingrate ='F' THEN bexrate ELSE sexrate END into l_result
        from mg_exrates
        where modifiedon = maxdate
...
CREATE FUNCTION {schema}.trgfunc_dimension_costcentre() RETURNS trigger
    LANGUAGE plpgsql
    SET search_path = {schema}, pg_catalog
    AS $$
BEGIN
	...
	if (TG_OP = 'UPDATE' or TG_OP = 'DELETE') then
		delete from costcentre_data a where a.sourceid = new.accountsdtlid;
	end if;

	if (TG_OP = 'UPDATE' or TG_OP = 'INSERT') then
		insert into costcentre_data
		(costcentre_dataid, sourceid, mapname,  
		accountname, subledgername, dimensions, costcentrename, amount, accountshdrid) 
		( 
/*			
			SELECT nextval('inbound_seq') as costcentre_dataid, new.accountsdtlid as sourceid, 
			'PROC' as mapname,  
			new.accountname, 
			new.subledgername, 
			new.dimensions,
			SPLIT_PART(a.dimension_data,'|',1) as costcentrename,
			cast(SPLIT_PART(a.dimension_data,'|',4) as numeric) as amount,
			new.accountshdrid
			from (
			select     
			unnest( string_to_array(new.dimension_data,',')) as dimension_data 
			) a
--			where (( new.dimension_data is not null or length( trim(new.dimension_data) )) !=0)  and new.dimensions != 'NA'
			where new.dimensions != 'NA'
*/ 
			...
			new.accountname, 
			new.subledgername, 
			cast(SPLIT_PART(a.dimension_data,'|',1) as varchar) as dimensions,
			SPLIT_PART(a.dimension_data,'|',2) as costcentrename,
			cast(SPLIT_PART(a.dimension_data,'|',5) as numeric) as amount,
			new.accountshdrid
			from (
			select     
			unnest( string_to_array(new.dimension_data,',')) as dimension_data 
